...
config_param = {"threshold_positive":75,"threshold_negative" : 75,"threshold_per_day" : 3,"threshold_time" : "12:00:00","threshold_hierarchy" : "weekly,monthly,alltime","flickr_api_key" : "shshgl","image_folder_path" : "/home/Users/data/images/food","path_pos_msg_users" : "/dashboard/config_data/","path_neg_msg_users" : "/dashboard/config_data/","path_neutral_msg_users" : "/dashboard/config_data/","path_weekly_msg_users" : "/dashboard/config_data/","path_monthly_msg_users" : "/dashboard/config_data/","path_alltime_msg_users" : "/dashboard/config_data/","path_spatial_stats_users": "/dashboard/config_data/","path_network_stats_users" : "/dashboard/config_data/","path_msg_img_users" : "/dashboard/config_data/","path_msg_table_users": "/dashboard/config_data/"}


positive_messages = []
negative_messages = []
pos_msg_friend = []
pos_msg_named_friend = []
pos_msg_network = []
pos_msg_similar = []

neg_msg_friend = []
neg_msg_named_friend = []
neg_msg_network = []
neg_msg_similar = []
...
def rest_visited_by_friends(rid,msg_type,restaurant_name=""):
	...
	for i in range(total_friends):
		...
		healthy_visits = cl.command("select name, res_id as res_id, steps_walked as steps from (select expand(both('eats_at')) from Person where @rid = "+friendid+")")
		healthy_visits_weekly = cl.command("select name, res_id as res_id, steps_walked as steps from (select expand(in) from eats_at where created_at > "+str(epoch_week)+" and out="+friendid+")")
		healthy_visits_monthly = cl.command("select name, res_id as res_id, steps_walked as steps from (select expand(in) from eats_at where created_at > "+str(epoch_month)+" and out="+friendid+")")
	...
	if (check_data_existence(all_friends_rest_visits)):
		...
		print "Counting steps walked all time"
		steps_walked = count_steps_walked(all_friends_healthy_visits)
...
#				print msg
			if index:
				...
				ret_val = begin_localimg(msg.replace("1 times","once"),"namedfriend_overlay.jpg")
				if ret_val == 1:
					recid_img = create_binary_data("/Local/Users/dev/NetworkLabs/working/namedfriend_overlay.jpg")
...
#				print msg
	...
	if (check_data_existence(all_friends_rest_visits_weekly)):
		...
		print "Computing weekly steps walked"
		steps_walked_weekly = count_steps_walked(all_friends_rest_visits_weekly)
		...
		if (msg_type==1):
			if steps_walked_weekly>=0:
				msg = 'Your friends have walked a total of '+str(steps_walked)+' steps over the past week'
				print msg
	...
	if (check_data_existence(all_friends_rest_visits_monthly)):
		...
		print "Computing the steps walked monthly"		
		steps_walked_monthly = count_steps_walked(all_friends_healthy_visits_monthly)
		...
		if msg_type==1:
			if steps_walked_weekly>=0:
				msg = 'Your friends have walked a total of '+str(steps_walked)+' steps over the past month'
				print msg
	...
	if (check_data_existence([rest_visits_weekly])):
		...
		if (msg_type==1):
			...
			tags = ["healthy","chinese"]
                        ...
                        if ret_val == 1:
                                        recid_img = create_binary_data("/Local/Users/dev/NetworkLabs/working/namedfriend_overlay_cuisine_monthly.jpg")
...
def create_profile_page_cards(userid):
	...
	msg1 = "This week, you have walked a total of <b>0 steps</b>"
	msg2 = "This week, you have eaten at <b>0 healthy restaurants</b>"
	msg3 = ""
	...
	cmd_str3 = 'create vertex Card set display_type="profile_info",sub_display_type="graphic",net_walk_avg="3560",you_walk_avg="2567",net_eat="4",you_eat="2",comment_count=0,comment_list=[],like_count=0,like_list=[],people_involved=[],created_at="'+str(int(creation_date.strftime('%s'))*1000)+'",created_at_datetime="'+str(creation_date.strftime('%Y-%m-%d'))+'",html_content="'+msg1+'"'
...
def set_global_var(config_file_path):
...
#	print root.attrib
	...
	for child in root:
		config_param[str(child.tag)]=str(child.text)
...
if __name__ == '__main__':
	config_file_path = "/home/chndrsh4/NetworkLabs/Dashboard/config_data/info_display_config.xml"
	#Read the config file generated by the dashboard and set the parameters
	set_global_var(config_file_path)
	'''
	cl = connect()
	print "Connected"
	peopleid = cl.command("select @rid from Person where first_name='Sanjana'")
...
#	print "Inserted"
	...
	# Named friend > friend> similar pref> network	
	for i in range(len(peopleid)):
		...
		print "Generating feedback card for user "
		gen_feedback_card(peopleid[i].rid)

		print "Generating profile page messages for user"
